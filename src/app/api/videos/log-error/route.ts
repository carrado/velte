...
// a client-side upload failure (see videoTrim.ts's trimVideoServerSide)
// would otherwise only ever show up in that one vendor's own DevTools
// console, invisible to anyone else. This just relays it into a
// console.error here so it lands in Vercel's Logs tab like any other
// server-side error — no new third-party service, just routing the report
// through infrastructure that's already there.
