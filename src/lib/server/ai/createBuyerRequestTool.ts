 ...
 * this same `description`/`buyerName` — no second AI turn required for
 * that part.
 */
export function createBuyerRequestTool(
  ...
  return tool({
    ...
    inputSchema,
    execute: async ({ description, buyerName }): Promise<BuyerRequestOffer> => {
      if (!buyerAuth) {
      ...
      try {
        ...
        const { created, request } = await backendData<{
          ...
          body: {
            ...
            // location and the vendor-facing page shows "N/A".
