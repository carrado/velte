...
function systemPromptFor(
  ...
  return [
    ...
    "- Only read a message as management when it does one of: names/implies a SPECIFIC existing plan or item, uses an explicit change verb (increase/reduce/remove/add/pause/resume/cancel/focus on) aimed at something already listed above, OR is a plain acknowledgment/closing remark with no shopping content of its own ('I think the list is fine', 'sounds good', 'that works', 'perfect thanks', 'ok great') — that third case is ALWAYS applies:true, action:'none', regardless of requestRelationHint, since a bare acknowledgment can't possibly be a new, unrelated shopping request. A message that merely resembles a listed plan's category, with no signal from any of these three, is not enough — leave applies false.",
